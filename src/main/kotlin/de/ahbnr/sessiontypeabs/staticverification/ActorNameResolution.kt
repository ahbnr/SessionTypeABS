...
private fun resolveActorNames(simpleNamesToClassDecls: Map<String, List<ClassDecl>>, type: GlobalType): GlobalType =
    when (type) {
        is GlobalType.Initialization ->
            type.copy(c = simpleToQualifiedActor(
                simpleNamesToClassDecls,
                type.c
            )
            )
        ...
        is GlobalType.Branching ->
            type.copy(
                choosingActor = simpleToQualifiedActor(
                    simpleNamesToClassDecls,
                    type.choosingActor
                ),
                branches = type
        ...
        is GlobalType.Repetition ->
            type.copy(
                repeatedType = resolveActorNames(
                    simpleNamesToClassDecls,
                    type.repeatedType
                )
            )
        ...
        is GlobalType.Interaction ->
            type.copy(
                caller = simpleToQualifiedActor(
                    simpleNamesToClassDecls,
                    type.caller
                ),
                callee = simpleToQualifiedActor(
                    simpleNamesToClassDecls,
                    type.callee
                )
            )
