...
import de.ahbnr.sessiontypeabs.types.AnalyzedLocalTypeVisitor
import de.ahbnr.sessiontypeabs.preprocessing.projection.exceptions.MethodProjectionException
...
class MethodProjector(
    ...
    // TODO: Loosen up the future equivalence requirements used here
    ...
    override fun visit(type: AnalyzedLocalType.ConcatenationType): MethodLocalType {
        val lhs =
            project(type.leftAnalyzedType, targetActor, targetFuture)
        val rhs =
            project(type.rightAnalzedType, targetActor, targetFuture)
    ...
    override fun visit(type: AnalyzedLocalType.OfferType): MethodLocalType {
        ...
        return if (targetFuture !in type.preState.getNonFreshFutures()) {
            val futureCreationBranches =
                type.analyzedBranches.filter {
                    targetFuture in it.postState.getNonFreshFutures()
                }
    ...
    override fun visit(type: AnalyzedLocalType.RepetitionType): MethodLocalType {
        ...
        return if (
            targetFuture !in type.preState.getNonFreshFutures() &&
            targetFuture in type.analyzedRepeatedType.postState.getNonFreshFutures()
        ) {
